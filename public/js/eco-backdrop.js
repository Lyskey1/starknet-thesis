/* The landing's ambient backdrop, ported to the static pages: the flame
   ...
   2026-09-08: the landing page mounts THIS file too (imported by the Next
   bundle), so the module exports mountBackdrop() and self-runs only when a
   page loads it as a script: a host that wants to call mount itself sets
   window.ECO_BACKDROP_MANUAL before importing. Nothing else changed; the
   static pages run the same code path they always did. */
...
export function mountBackdrop() {
  ...
  return {
    ...
    dispose() {
      ...
      motes.geometry.dispose(); motes.material.dispose(); wash.geometry.dispose(); wash.material.dispose();
      renderer.dispose();
...
if (!window.ECO_BACKDROP_MANUAL) mountBackdrop();
