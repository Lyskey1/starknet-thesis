/* STRK hero centrepiece: a glowing particle Starknet mark inside a particle
   ring, chalk at the top fading to the page accent at the bottom.

   This is the twin of js/quantum-hero-mark.js and js/btcfi-hero-mark.js, the
   static-page ports of the landing hero's WebGL orb. One object filling the
   middle of the frame, with the copy placed around its edges. It replaces the
   node-and-lines convergence fan that used to sit in this hero's right zone.

   THE GLYPH IS assets/img/starknet-mark.svg, the OFFICIAL MARK WITH ITS DISC,
   deliberately NOT the disc-less starknet-glyph.svg /quantum samples, so the
   two pages do not render the same centrepiece. The disc gives this one a
   solid coin silhouette against quantum's open sparkle-and-swoosh cloud.

   Mount is #skhStage, an absolutely positioned box inside .hero. The renderer
   ...
   The svg ships with a viewBox and NO width/height, so an <img> of it measures
   0x0 in Chrome and anything built from it comes back blank. The source is
   fetched and an explicit width/height is stamped on the ROOT element before
   it is rasterised into a 2D canvas. Nothing is ever injected into a <path>:
   ...
   starknet-mark.svg carries no root fill (its paths are self-coloured), so a
   fill IS stamped on the ROOT alongside the size. That is the safe element for
   it, and it guarantees a non-blank raster whatever the paths declare.

   THE RASTER IS ASSERTED, NOT ASSUMED. The lit-pixel count is checked before
   any geometry is built, the module bails loudly if it is zero, and the final
   particle count is published on window.__skh so a browser can read it back
   rather than trusting that the fetch worked.
   ...
   SPEED matches /quantum's slow drift: the ring turns at about 1 degree per
   second and reads as a drift rather than a spin.

   strk20 palette only: accent #c53400, warm #e07a4a, chalk #fafafa. */
import * as THREE from 'three';

const MOUNT = document.getElementById('skhStage');
if (MOUNT) {
  ...
  catch (e) { console.error('[strk-hero-mark] no WebGL context, the hero renders without the centrepiece', e); }
  ...
  if (renderer) {
    ...
    fetch('/assets/img/starknet-mark.svg')
      ...
      .then((src) => {
        /* width/height stamped on the ROOT; the file already carries its fill
           ...
           header note. */
        /* WIDTH AND HEIGHT ON THE ROOT, and NOTHING ELSE. The file's five
           paths carry their own fills (#0C0C4F disc, #EC796B and #FAFAFA
           mark), and those fills are what the sampler below reads to tell the
           disc from the mark, so a root fill would flatten the whole thing to
           one solid colour and the mark would rasterise as a featureless
           ball. Nothing is injected into a <path> either: a fill on a <path>
           yields an SVG Chrome silently refuses to decode. Both traps are in
           the header note. */
      ...
      .then((img) => {
        ...
        const TH = 110;
        /* LUMINANCE, NOT JUST ALPHA. This glyph is the official mark WITH its
           disc, and the disc is opaque: threshold on alpha alone and every
           pixel inside the coin is a candidate at equal weight, which is what
           rasterises as a featureless ball (measured: 206,034 identical
           hits). So each lit pixel also carries its perceived luminance. The
           #0C0C4F disc lands near 0.05 and becomes sparse dim dust; the
           #FAFAFA and #EC796B mark paths land near 0.98 and 0.60 and become
           the bright drawing inside it. That is the whole reason this page
           can sample the disc variant and still read as the Starknet mark
           rather than as quantum's cloud with a fill behind it. */
        const LUM_TH = 0.30;
        const hits = [], edge = [], lum = [];
        ...
        for (let y = 0; y < S; y++) {
          for (let x = 0; x < S; x++) {
            if (A(x, y) > TH) {
              ...
              hits.push(x, y); edge.push(isEdge ? 1 : 0); lum.push(L);
              if (x < x0) x0 = x;
              if (x > x1) x1 = x;
              if (y < y0) y0 = y;
              if (y > y1) y1 = y;
        ...
        /* ASSERT, do not assume: a blank raster is the documented failure mode
           of this pipeline, so the count is checked before any geometry is
           built and published for a browser to read back. */
        window.__skhLit = total;
        if (!total) { console.error('[strk-hero-mark] the mark rasterised blank'); return; }
        ...
        /* the budget goes where the drawing is: every bright pixel (the mark)
           and every rim pixel is walked whole, and the dark disc behind them
           is decimated hard so it reads as dust rather than as fill. */
        const DISC_KEEP = 0.10;
        const keep = [];
        for (let j = 0; j < total; j++) {
          if (lum[j] > LUM_TH || edge[j]) keep.push(j);
          else if (rnd(j, 45.164) < DISC_KEEP) keep.push(j);
        }
        ...
        for (let i = 0; i < N; i++) {
          ...
          const k = j * 2, isEdge = edge[j], bright = lum[j] > LUM_TH;
          const jx = (rnd(i, 127.1) - 0.5) * 1.5, jy = (rnd(i, 311.7) - 0.5) * 1.5;
          pos[i * 3] = (hits[k] + jx - mx) * scale;
          pos[i * 3 + 1] = (my - (hits[k + 1] + jy)) * scale;
          /* the disc is pushed a little behind the mark so the coin reads as
             a shell with the drawing floating in front of it */
          pos[i * 3 + 2] = (rnd(i, 39.42) - 0.5) * 0.16 - (bright ? 0 : 0.22);
          siz[i] = (bright ? 1.35 : isEdge ? 1.05 : 0.62) + rnd(i, 12.9898) * 0.80;
          pha[i] = rnd(i, 78.233);
          alp[i] = (bright ? 0.88 : isEdge ? 0.46 : 0.16) + rnd(i, 91.3) * (bright ? 0.40 : 0.16);
          tin[i] = (bright ? 1.10 : 0.72) + rnd(i, 5.331) * 0.35;
        }
        markGrp.add(points(pos, siz, pha, alp, tin, makeMaterial(1, GLYPH_H)));
        /* the verification hook the report reads: lit pixels found in the
           raster, and the particles actually uploaded to the GPU */
        let brightHits = 0;
        for (let j = 0; j < total; j++) if (lum[j] > LUM_TH) brightHits++;
        Object.assign(window.__skh || (window.__skh = {}),
          { lit: total, bright: brightHits, kept: keep.length, particles: N, ring: RING_N + HALO_N });
        fit();
        if (reduced) draw(0);
      })
      .catch((e) => console.error('[strk-hero-mark] the mark failed to load', e));
    ...
    /* verification hook, always on and cheap: the raster is ASSERTED rather
       than assumed, so a browser can read the lit-pixel and particle counts
       straight off the scene instead of trusting that the fetch worked. The
       scene objects ride along so the drift rate can be sampled too. */
    window.__skh = Object.assign({ lit: 0, particles: 0 },
      { world, ringGrp, markGrp, SPEED });
  }
}
