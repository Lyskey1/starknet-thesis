/* DIGEST hero centrepiece: a glowing particle envelope-and-issue mark inside a
   particle ring, chalk at the top fading to the page accent at the bottom.

   This is the twin of js/quantum-hero-mark.js and js/btcfi-hero-mark.js, both
   ...
   tiled mono word field (ROUNDUP / RECAP / WEEKLY, built by an inline script
   at the foot of digest.html, retired 2026-09-02) that used to run here, and
   the white knockout band that sat in the headline over it.

   THE GLYPH IS THE ARCHIVE ITSELF: an envelope with an issue behind it,
   sampled from assets/img/digest-glyph.svg. No suitable mark existed in
   assets/img, so that file was drawn for this: outlined shapes rather than
   solid ones, because the point cloud reads as a drawing only if there are
   edges in it.

   Mount is #dghStage, an absolutely positioned box inside .hero. The renderer
   ...
   assets/img/digest-glyph.svg ships with a viewBox and NO width/height, so an
   ...
   ROOT element before it is rasterised into a 2D canvas. The file carries its
   fill and its fill-rule on the ROOT too, so nothing is ever injected into a
   <path>: a fill on a <path> yields an SVG Chrome silently refuses to decode.
   Both traps are documented in js/btcfi-globe.js and js/pv-eco-orb.js and
   have bitten more than once; do not improvise a variant.
   ...
   count. A blank raster is a HARD ERROR, not a silent empty canvas, and the
   surviving counts are published on window.__dgh for /digest?dghprobe to read
   back, which is how the raster is asserted non-blank rather than assumed.
   Because the fetch is async the geometry is built INSIDE the callback: no
   empty buffer is created up front and filled later.
   ...
   GATING is the quantum twin's, unchanged: an IntersectionObserver on the
   hero plus visibilitychange, so nothing runs off screen or in a hidden tab,
   and under prefers-reduced-motion one settled frame is painted instead of
   animating.

   strk20 palette only: accent #c53400, warm #e07a4a, chalk #fafafa. */
import * as THREE from 'three';

const MOUNT = document.getElementById('dghStage');
if (MOUNT) {
  ...
  catch (e) { console.error('[digest-hero-mark] no WebGL context, the hero renders without the centrepiece', e); }
  ...
  if (renderer) {
    ...
    const RING_N = small ? 6400 : 25000;
    const HALO_N = small ? 1800 : 7400;
    ...
    /* ================= the mark ================= */
    const MARK_N = small ? 11000 : 44000;
    fetch('/assets/img/digest-glyph.svg')
      .then((r) => r.text())
      .then((src) => {
      ...
      .then((img) => {
        ...
        if (!total) { console.error('[digest-hero-mark] the mark rasterised blank'); return; }
        ...
        /* asserted, not assumed: the blank-raster case is a hard error above,
           and the counts are published for the probe to read back. */
        window.__dgh = Object.assign(window.__dgh || {}, { lit: total, marks: N });
        fit();
        if (reduced) draw(0);
      })
      .catch((e) => console.error('[digest-hero-mark] the mark failed to load', e));
    ...
    function fit() {
      ...
      /* the mark size rule (2026-09-05, shared with strk/btcfi/quantum):
         the annulus' outer edge spans 80% of the mount height, and the mount
         is the full scene band, so the ring diameter is 80% of the column
         between the nav and the baseline strip. byW guards the stage box on
         a narrow desktop. */
      ...
      /* CENTRED, unlike the quantum twin. That hero runs a three-line display
         down the left gutter and pushes its object right to clear it; this
         one parks its copy in the four corners of the frame, so the middle
         is free and the object sits in it. */
      world.position.x = 0;
      camera.updateProjectionMatrix();
      const pix = Math.min(devicePixelRatio || 1, 2);
      const trim = sm ? 1 : 0.82;
    ...
    /* verification hook, off unless asked for: /digest?dghprobe exposes the
       scene so a headful browser can read the mark's real particle count and
       the drift rate off the objects rather than off these constants. The
       raster counts are filled in by the loader above (window.__dgh.lit is
       the number of pixels over the alpha threshold, .marks the number that
       became particles), which is what proves the glyph is not blank. */
    if (location.search.includes('dghprobe')) {
      window.__dgh = Object.assign(window.__dgh || {}, { world, ringGrp, markGrp, SPEED, ring: RING_N + HALO_N });
    }
  }
}
